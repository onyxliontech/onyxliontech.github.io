...

      {
        data.map((item) => (
          <div className="flex flex-col mt-8 justify-center">
            <Fade bottom triggerOnce>
              <div className="flex justify-center"><a href={item.links} target="_blank" rel="noopener noreferrer" className="text-5xl text-theme-blue self-center text-center font-bold">{item.title}</a></div>
            ...
            <Fade bottom delay={300 * 1} triggerOnce>
              ...
              <p className="font-light italic text-gray-400 mt-16 mx-8 sm:mx-16 xl:mx-28">
                ...
                Click here to view  :
                {' '}
                <Button type="link" href={item.links} target="_blank" className="text-theme-purple" isExternal>{item.title}</Button>
              </p>
            </Fade>
          </div>
        ))
